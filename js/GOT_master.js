...
`],
  ...
  function playPauseVideo() {
  	if (houseVideo.paused || houseVideo.ended) {
  ...
  function updateProgressBar() {
  	...
  	var percentage = Math.floor((100 / houseVideo.duration) * houseVideo.currentTime);
  ...
  function popLightBox(){
    ...
    // debugger;
    // get the class name property split it into its separate words and the get the last word--> [1] that will always be the house name
    ...
    // use java script string interpolation to get file name
  ...
  function animateBanners(){
    // we need an offset that we can multiply by to animate our banners to the left and make the active one show up

    let offset = 600, multiplier = this.dataset.offset;
...
// change house name on the page at same time
...
houseInfo.textContent = houseData[multiplier][1];
  ...
  houseVideo.addEventListener('timeupdate', updateProgressBar, false);


  houseVideo.addEventListener('ended', closeLightBox);
  volumeBar.addEventListener("change", function(evt) {
		houseVideo.volume = evt.target.value;
	});
})();
